...
Info
    Defines class GenKernelModule with attribute(s) and method(s).
...
class GenKernelModule(ATSCli):
    '''
        ...
        It defines:
            ...
            :methods:
                | __init__ - Initials GenKernelModule constructor.
                | process - Processes and runs operations.
    '''

    _GEN_VERBOSE: str = 'GEN_KERNEL_MODULE'
    _CONFIG: str = '/conf/gen_kernel_module.cfg'
    _LOG: str = '/log/gen_kernel_module.log'
    _LOGO: str = '/conf/gen_kernel_module.logo'
    ...
    def __init__(self, verbose: bool = False) -> None:
        ...
        if self.is_operational():
            ...
            self.add_new_option(
                ...
                help='driver type (char | block | net | vma)'
            )
            self.add_new_option(
        ...
        if self.is_operational():
            try:
                ...
                if exists(f'{getcwd()}/{str(getattr(args, "name"))}'):
                    ...
                    return status
                generator: GenLKM = GenLKM(getattr(args, 'verbose') or verbose)
                try:
                    print(
                        " ".join([
                            f'[{self._GEN_VERBOSE.lower()}]',
                            'generation linux kernel skeleton',
